...
| TimelineFeed | タイムライン表示 | feedType, posts[], isRefreshing |
| UserProfile | プロファイル表示・編集 | user, posts[], isEditing |
| PinCard | Pin情報カード表示 | pin, onBookmark, onNavigate |
| PostCarousel | 日別投稿カルーセル | dailyPosts[], currentIndex |
| TagFilter | タグフィルターUI | availableTags[], selectedTags[] |

### APIエンドポイント

| メソッド | ルート | 目的 | 認証 | ステータスコード |
|---------|--------|------|------|------------------|
| POST | /api/auth/magic-link | マジックリンク送信 | 不要 | 200, 400, 429, 500 |
| POST | /api/auth/verify | マジックリンク検証 | 不要 | 200, 400, 401, 500 |
| GET | /api/profile/:userId | プロファイル取得 | 必要 | 200, 401, 404, 500 |
| PUT | /api/profile | プロファイル更新 | 必要 | 200, 400, 401, 500 |
| POST | /api/posts | 投稿作成 | 必要 | 201, 400, 401, 500 |
| GET | /api/posts/:postId | 投稿詳細取得 | 条件付き | 200, 401, 403, 404, 500 |
| DELETE | /api/posts/:postId | 投稿削除 | 必要 | 204, 401, 403, 404, 500 |
| GET | /api/timeline/:type | タイムライン取得 | 必要 | 200, 401, 500 |
| GET | /api/pins/nearby | 周辺Pin検索（PostGIS） | 必要 | 200, 401, 500 |
| GET | /api/poi/search | POI検索（外部プロバイダ正規化） | 必要 | 200, 401, 500 |
| GET | /api/poi/details | POI詳細（source+source_place_id） | 必要 | 200, 401, 404, 500 |
| POST | /api/pins | Pin作成 | 必要 | 201, 400, 401, 500 |
| POST | /api/pins/:pinId/bookmark | Pinブックマーク | 必要 | 201, 401, 404, 500 |
| POST | /api/social/follow | フォロー | 必要 | 201, 401, 404, 500 |
| POST | /api/social/friend-request | 友達申請 | 必要 | 201, 401, 404, 500 |
...
#### 2. POI詳細取得エンドポイント
```typescript
// GET /api/poi/details?source={source}&source_place_id={id}&mode={mode}
interface POIDetailsRequest {
  source: string;          // データソース
  source_place_id: string; // 外部プロバイダID
  mode: 'card' | 'details'; // 取得モード（FieldMask制御用）
  session_token?: string;  // Autocomplete連携用（セッション課金）
}
...
```typescript
// GET /api/pins/nearby?lat={lat}&lng={lng}&radius={radius}
...
const selectPlace = async (placeId: string, token: string) => {
  const details = await fetch(
    `/api/poi/details?source=google&source_place_id=${placeId}&mode=card&session_token=${token}`
  );
...
--       display_name = NULL, bio = NULL, header_image_url = NULL, avatar_image_url = NULL
-- WHERE id = :user_id;
...
SELECT id, username, display_name, avatar_image_url
FROM profiles
WHERE deleted_at IS NULL; -- 削除済みユーザーを除外
...
const VideoPlayer = ({ videoUid, isPrivate = false }) => {
  ...
  useEffect(() => {
    const getVideoUrl = async () => {
      if (isPrivate) {
        // 署名付きURL取得
        const response = await fetch(`/api/video/${videoUid}/signed-url`);
...
- **水平スケーリング**: Kubernetes/ECSによるコンテナオーケストレーション
- **データベース**: 読み取りレプリカ + コネクションプーリング
- **非同期処理**: Bull/BullMQによるジョブキュー（画像処理、通知送信）
...
describe('PostService', () => {
  it('should create post with location', async () => {
    const mockPost = {
      caption: 'テスト投稿',
...
```javascript
describe('投稿作成フロー', () => {
  it('写真を選択して投稿できる', async () => {
    await element(by.id('post-button')).tap();
    await element(by.id('camera-roll')).tap();
    await element(by.id('photo-1')).tap();
    await element(by.id('caption-input')).typeText('楽しい一日');
    await element(by.id('submit-post')).tap();
    await expect(element(by.text('投稿完了'))).toBeVisible();
  });
});
```
...
#### 週1-2: 基盤構築
1. Supabase基盤構築（認証・データベース・ストレージ）
2. React Native + Expo プロジェクト初期化
3. マジックリンク認証システム

#### 週3-4: 画像投稿機能
1. 画像アップロード（原本保存）
2. Supabase Storage Image Transformationsによる画像変換システム
3. 基本的な投稿機能（写真・キャプション）

#### 週5-6: タイムライン機能
1. シンプルなタイムライン表示
2. 最適化された画像配信（CDN統合）
3. 基本的なマップ表示（Google Maps SDK統合）

#### 週7-8: 動画対応・完成度向上
1. Cloudflare Stream統合（動画アップロード・再生）
2. GPS/EXIF位置情報からのPin自動生成
3. 基本的なプロファイル機能
4. MVP版テスト・デバッグ
...
#### 週9-10: Pin・ソーシャル機能
1. Pin管理システム（作成・検索・詳細表示）
2. ソーシャル機能（フォロー/友達）
3. 公開範囲制御（Public/Friends/Private）

#### 週11-12: 高度な機能
1. 24時間投稿レビュー機能
2. タグフィルター・検索機能
3. ブックマーク（Pinning）機能
4. タイムラインの3タブ（featured/follow/friend）
...
const costEvolution = {
  mvp: {
    users: '< 100',
    monthlySpend: '$5-10',
    strategy: 'Supabase Storage + Edge Functions'
  },
  growth: {
    users: '100-1000', 
    monthlySpend: '$20-50',
    strategy: 'CDN最適化 + Cloudflare Stream本格運用'
  },
  scale: {
    users: '1000+',
    monthlySpend: '$100+',
    strategy: 'ImageKit等導入検討 + インフラスケーリング'
  }
};
```